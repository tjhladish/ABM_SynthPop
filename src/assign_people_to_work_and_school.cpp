...
const double student_teacher_ratio = 28/2.0; // x2 is for support staff
...
vector<HouseType*> import_households(string filename_in, string filename_out) {
    vector<HouseType*> hh_loc;
    cerr << "reading household locations\n";
    istringstream line;
    ifstream fh_in(filename_in);
    ofstream fh_out(filename_out);
    fh_out << fh_in.rdbuf();
    fh_out.close();
    fh_in.seekg(0);
    ...
    if (fh_in) {
        ...
        while (getline(fh_in, buffer)) {
          ...
          /*id type x y
            0 h -89.6910220003 20.7015302009
            1 h -89.700145483 20.6641877526
            2 h -89.758249035 20.6954360352
            3 h -89.6974011142 20.6551249287*/
...
vector<LocationType*> import_workplaces_and_schools(const string filename, const int hh_ct, map<int, int> &workplace_lookup, double &total_raw_workers) {
    ...
    string buffer;
    ofstream fh_out("../pop/locations-yucatan.txt", ofstream::app);
    if (fh_in) {
        while (getline(fh_in, buffer)) {
            line.clear();
            line.str(buffer);
...
vector<PersonType*> import_population(string filename, vector<HouseType*>& hh_loc, map<char, int>& day_loc_ctr, mt19937& rng) {
    ...
    map<int, vector<int>> age_by_empstat; // some people have ages of 999; this is to sample a new age based on employment status 
    ...
    if (fh_in) {
       ...
       /*
        pid hid age sex workid empstatd
        1 1 31 1 -1 110
        2 1 29 2 -1 110
        3 1 10 2 -1 0
        4 2 32 1 -1 110
       */
            ...
            if (line >> pid >> hid >> age >> sex >> workid >> empstat) {
                ...
                if (age_by_empstat.count(empstat) == 0) age_by_empstat[empstat] = vector<int>();
                if (age >= 0 and age <= 100) age_by_empstat[empstat].push_back(age);
    ...
    for (PersonType* p: pop) {
        if (p->age >= 0 and p->age <= 100) {
            continue;
        } else {
            // sample a random age from someone with the same employment status who has a valid age
            const int empstat = p->empstat;
            assert(age_by_empstat.count(empstat) > 0);
            assert(age_by_empstat[empstat].size() > 0);
            uniform_int_distribution<int> runif(0, age_by_empstat[empstat].size() - 1);
            p->age = age_by_empstat[empstat][runif(rng)];
        }
    }
...
int main() {
    ...
    vector<HouseType*> hh_loc = import_households("../derived_data/locations-yucatan-prelim.txt", "../pop/locations-yucatan.txt");
    ...
    vector<LocationType*> workplaces_and_schools = import_workplaces_and_schools("../derived_data/workplaces_and_schools.out", hh_loc.size(), workplace_lookup, total_raw_workers);
    cerr << workplaces_and_schools.size() << endl;
    ...
    vector<PersonType*> pop = import_population("../derived_data/population-yucatan-prelim.txt", hh_loc, day_loc_ctr, rng);
    ...
    ofstream fh_out("../pop/population-yucatan.txt");
    fh_out << "pid hid age sex workid empstat" << endl;
    ...
    for (PersonType* p: pop) {
        ...
        fh_out << p->pid << " " << p->hid << " " << p->age << " " << p->sex << " " << p->workid << " " << p->empstat << endl;
