...
bool loadPopulation(string popFilename) {
    ...
    while (iss) {
        ...
        int id, age, hid, empstat;
        long int workid;
        string sex;
        if (line >> id >> hid >> age >> sex >> workid >> empstat) {
            Person* peep = new Person();
            peep->id = id;
            peep->loc = locations[hid];
            peep->age = age;
            peep->sex = sex;
            people.push_back(peep);
            people_by_age[age].push_back(peep);
        }
    }
    iss.close();
...
bool loadLocations(string locFilename) {
    ...
    id type x y
    0 h 0.327530618291348 0.324621923966333
    1 h 0.849513080669567 0.304251517867669
    2 h 0.00842146878130734 0.657222841167822
    3 h 0.369299583602697 0.349579052301124
    4 h 0.460470566991717 0.277130988193676
    5 h 0.0256796989124268 0.712766533251852
    6 h 0.431369476253167 0.20579392160289
    7 h 0.355606281897053 0.540817143395543
    8 h 0.996641094330698 0.151982805691659
    */
    ...
    while (iss) {
        ...
        if (line >> locID >> type >> xcoord >> ycoord) {
            ...
            loc->x = xcoord;
            loc->y = ycoord;
            if (locations.size() != locID) {
                cerr << "something is screwed up with location ID's and indexing\n";
                exit(-1);
            }
...
int main(int argc, char** argv) { 
    ...
    const int NUM_PEOPLE = 1817734; // yucatan
  ...
  //const int NUM_PEOPLE =  839659; // merida
    ...
    #pragma omp parallel for num_threads(4) // change num_threads based on available cores
    ...
    for(int i = 0; i < people.size(); i++) {
        ...
        // Calculate the haversine distances from this person to all individuals
